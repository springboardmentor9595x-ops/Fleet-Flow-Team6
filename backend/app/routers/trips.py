...
def schedule_trip_route(data: dict = Body(...), db: Session = Depends(get_db)):
    return schedule_trip(data=data, db=db)

@router.post("/{trip_id}/start")
@router.put("/{trip_id}/start")
def start_trip_route(trip_id: str, db: Session = Depends(get_db)):
    return start_trip(trip_id=trip_id, db=db)

@router.post("/{trip_id}/end")
@router.put("/{trip_id}/end")
def end_trip_route(trip_id: str, db: Session = Depends(get_db)):
    return end_trip(trip_id=trip_id, db=db)

@router.post("/{trip_id}/recalculate-route")
@router.post("/{trip_id}/recalculate")
def recalculate_route_endpoint(trip_id: str, db: Session = Depends(get_db)):
